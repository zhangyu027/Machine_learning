"""Legacy/demo multi-endpoint ADMET decision-support layer.

This module intentionally uses transparent proxy functions so the application can
run without trained model artifacts. It did NOT generate the V4.1 scientific
multi-task benchmark. The trainable scientific architecture is exposed separately
as ``pharma_genai.models.multitask_mlp.MultiTaskMorganMLP`` and benchmarked by
``experiments/run_multitask_admet.py``.
"""
...
class MultiTaskADMETPredictor:
    """Backward-compatible heuristic ADMET predictor for demo/application flows."""
