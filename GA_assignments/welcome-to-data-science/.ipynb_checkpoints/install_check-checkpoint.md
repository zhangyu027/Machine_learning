...
If you use Windows, click [here to download the Git Bash shell](https://gitforwindows.org/). This will allow you to emulate most of the common commands and functions native to OS and Linux systems.
...
4) Once installed, run the following command to create a new environment. Any packages that you install in this environment will be limited to it. This can help you to manage multiple projects on one machine.
```bash
conda create --name ga python==3.7
```
...
Since this command will need to be run each time you launch a new shell, you may choose to add an alias to it in your .bash_profile file, for e.g.,

```bash
alias cag='conda activate ga'
```

5) In the newly created environment, run the following command to ensure that some frequently used libraries are installed. Anaconda may also update your packages at this time (which is OK!). <br>
...
These identifiers will be added to your commits and show up when you push your changes to [GitHub](https://git.generalassemb.ly)!

### Optional: Set Up SSH for Easier Remote Connection

While you can connect your local repositories (the work on your laptop) to remote repositories (those stored on [GitHub](https://git.generalassemb.ly)) without much additional effort, this may prompt you to input your username and password quite  frequently. However, there's an alternative known as SSH, which will let you create a file on your computer that will authenticate you to [GitHub](https://git.generalassemb.ly) without entering your username and password over and over again.
